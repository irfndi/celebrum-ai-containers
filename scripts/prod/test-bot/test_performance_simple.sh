...
measure_response() {
    ...
    local response=$(curl -X GET "$url" -s -w "%{http_code}" -o /dev/null 2>/dev/null)
...
# Main execution
main() {
    log "${CYAN}🚀 Simple Performance Test${NC}"
    ...
    # Run simple tests
    simple_concurrent_test "Health Check Test" "$BASE_URL/api/v1/health" 5 3
    simple_concurrent_test "User Profile Test" "$BASE_URL/api/v1/users/profile" 3 2
    
    log "${GREEN}🎉 Simple performance testing completed!${NC}"
}
