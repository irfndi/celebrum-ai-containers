...
export class ArbitrageOpportunityManager {
  ...
  async createOpportunityAlert(
    ...
    try {
      ...
      const existingAlerts = await this.env.CELEBRUM_KV?.get(userAlertsKey, 'json') || [];
  ...
  async getUserOpportunityAlerts(userId: string): Promise<any[]> {
    try {
      const userAlertsKey = `rbac:user_alerts:${userId}`;
      const alertIds = await this.env.CELEBRUM_KV?.get(userAlertsKey, 'json') || [];
  ...
  private async resetCountersIfNeeded(
    ...
    try {
      ...
      const lastReset = await this.env.CELEBRUM_KV?.get(lastResetKey, 'json') || {
        dailyReset: 0,
        hourlyReset: 0
      };
  ...
  private async getOpportunitiesFromCache(role: UserRoleType): Promise<ArbitrageOpportunity[]> {
    try {
      ...
      const cached = await this.env.CELEBRUM_KV?.get(cacheKey, 'json');
