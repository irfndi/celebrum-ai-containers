...
import { users, userUsernameHistory, positions, opportunities, tradingStrategies, invitationCodes, invitationUsage } from '../../src/schema/index.js';
import { DatabaseQueries, UserUsernameHistoryQueries } from '../../src/utils/queries.js';
import { getTestDb, cleanupDb, testData } from '../../../shared/tests/utils/test-helpers.ts';
...
describe('Database Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    dbQueries = new DatabaseQueries(testDb);
  });
  ...
  beforeEach(async () => {
    ...
    // Reset mock data between tests for proper data isolation
    if (testDb && typeof testDb.resetData === 'function') {
      testDb.resetData();
      // Always call init() after resetData() to ensure proper initialization
      if (typeof testDb.init === 'function') {
        await testDb.init();
      }
    }
  });
  ...
  describe('User Operations', () => {
    ...
    it('should find user by telegram ID', async () => {
      // Create user first
      const createdUser = await dbQueries.users.create(testUser);
      vi.spyOn(testDb.query.users, 'findFirst').mockResolvedValue(createdUser);
    ...
    it('should delete user', async () => {
      ...
      vi.spyOn(testDb.query.users, 'findFirst').mockResolvedValue(null);
      const foundUser = await dbQueries.users.findById(createdUser.id);
  ...
  describe('User Username History Operations', () => {
    ...
    it('should create username history entry', async () => {
      const historyQueries = new UserUsernameHistoryQueries(testDb);
      const historyData = {
        userId,
    ...
    it('should find username history by user ID', async () => {
      ...
      expect(history.map(h => h.username)).toContain('username1');
      expect(history.map(h => h.username)).toContain('username2');
    });
  ...
  describe('Position Operations', () => {
    ...
    it('should find positions by user ID', async () => {
      ...
      expect(positions.map(p => p.symbol)).toContain('BTC/USDT');
      expect(positions.map(p => p.symbol)).toContain('ETH/USDT');
    });
  ...
  describe('Transaction Handling', () => {
    it('should handle database transactions', async () => {
      ...
      try {
        await testDb.transaction(async (tx: any) => {
          // Create user
  ...
  describe('Data Integrity', () => {
    it('should maintain referential integrity', async () => {
      ...
      try {
        ...
        const usernameHistoryQueries = new UserUsernameHistoryQueries(testDb);
  ...
  describe('Data Consistency Tests', () => {
    it('should maintain data consistency across related tables', async () => {
      ...
      const allPositions = testDb.mockData.positions || [];
      const userPositions = await dbQueries.positions.findByUserId(user.id);
      
      if (userPositions.length !== 1) {
        throw new Error(`Expected 1 position, found ${userPositions.length}. All positions: ${JSON.stringify(allPositions.map(p => ({ id: p.id, userId: p.userId })))}. Created position: ${JSON.stringify({ id: position.id, userId: position.userId })}. Looking for userId: ${user.id}. Found positions: ${JSON.stringify(userPositions.map(p => ({ id: p.id, userId: p.userId })))}`);
      }
    ...
    it('should handle concurrent updates correctly', async () => {
      ...
      if (!foundPosition) {
        ...
        const directResult = await testDb.select().from({ name: 'positions' }).where({ id: position.id });
