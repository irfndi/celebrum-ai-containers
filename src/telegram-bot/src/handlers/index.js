...
exports.TELEGRAM_HANDLERS = void 0;
...
const shared_1 = require("@celebrum-ai/shared");
const db_1 = require("@celebrum-ai/db");
const errors_1 = require("@celebrum-ai/shared/errors");
// Command handlers registry
exports.TELEGRAM_HANDLERS = new Map();
// Register a command handler
function registerHandler(handler) {
    exports.TELEGRAM_HANDLERS.set(handler.command, handler);
}
// Get handler for command
function getHandler(command) {
    return exports.TELEGRAM_HANDLERS.get(command);
}
// Get all registered handlers
function getAllHandlers() {
    return Array.from(exports.TELEGRAM_HANDLERS.values());
}
...
async function processTelegramUpdate(update, context) {
    try {
        ...
        const command = (0, index_1.extractCommand)(message.text);
        if (!command) {
            return null;
        }
        // Rate limiting check
        const userId = (0, index_1.getUserId)(update);
        if (userId && (0, index_1.isRateLimited)(userId)) {
            const chatId = (0, index_1.getChatId)(update);
            if (chatId) {
                return {
                    method: 'sendMessage',
                    chat_id: chatId,
...
async function processCallbackQuery(update, context) {
    ...
    try {
        ...
        if (response) {
            ...
            context.waitUntil(fetch(`https://api.telegram.org/bot${context.env.TELEGRAM_BOT_TOKEN}/answerCallbackQuery`, {
                method: 'POST',
...
function initializeHandlers() {
    ...
    registerHandler({
        ...
        handler: async (update, context) => {
            const sessionService = new shared_1.SessionService(context.env.SESSIONS);
            const chatId = (0, index_1.getChatId)(update);
            const from = update.message?.from;
            if (!chatId || !from)
                return null;
            const db = (0, db_1.createDb)(context.env.DB);
            const userService = new shared_1.UserService(db);
            const invitationService = new shared_1.InvitationService(db);
    ...
    registerHandler({
        ...
        handler: async (update, context) => {
            const chatId = (0, index_1.getChatId)(update);
            const _userId = (0, index_1.getUserId)(update);
            if (!chatId)
                return null;
            ...
            const from = update.message?.from;
            if (from) {
                const adminIds = (context.env.ADMIN_TELEGRAM_IDS || '').split(',').map((id) => parseInt(id.trim(), 10)).filter((id) => !isNaN(id));
