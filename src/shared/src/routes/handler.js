...
class RouteHandler {
     ...
     */
    constructor(services, fallbackService) {
     ...
     */
    async handleUnknownRoute(request) {
     ...
     */
    constructServiceUrl(service, originalUrl) {
     ...
     */
    notImplementedResponse(feature) {
     ...
     */
    maintenanceResponse(message, estimatedResolution) {
        ...
        if (estimatedResolution) {
            const secondsUntilResolution = Math.max(0, Math.floor((estimatedResolution.getTime() - Date.now()) / 1000));
