...
exports.ApiAccessManager = void 0;
/**
...
class ApiAccessManager {
     ...
     */
    async addExchangeApi(userId, exchangeId, apiKey, apiSecret, passphrase, sandbox = false) {
        try {
     ...
     */
    async addAiApi(userId, provider, apiKey, model, endpoint) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     * Remove exchange API
     */
    async removeExchangeApi(userId, exchangeId) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     * Remove AI API
     */
    async removeAiApi(userId, provider) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     */
    async getExchangeApiCredentials(userId, exchangeId) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     */
    async getAiApiCredentials(userId, provider) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     */
    async checkRateLimit(userId, window = 'hourly') {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     */
    async getApiAccessSummary(userId) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
     ...
     */
    async toggleApiStatus(userId, type, identifier, isActive) {
        try {
            const key = `rbac:api_access:${userId}`;
            const apiAccess = await this.env.CELEBRUM_KV?.get(key, 'json');
            if (!apiAccess) {
...
exports.ApiAccessManager = ApiAccessManager;
//# sourceMappingURL=api-access-manager.js.map