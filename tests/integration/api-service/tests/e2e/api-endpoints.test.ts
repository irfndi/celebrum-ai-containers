...
import { getTestDb, createMockEnv, createMockContext, createMockRequest, createMockResponse, createMockUser } from '../../../../../src/shared/tests/utils/test-helpers';
import type { DrizzleD1Database } from 'drizzle-orm/d1';
import * as schema from '../../../../../src/db/src/schema';
import type { User, ExtendedKVNamespace } from '../../../../../src/shared/src/types';

// Remove all ad-hoc mockDatabase, mockExchangeAPI, mockAlchemyAPI, mockJWT
// Use shared helpers for all DB, KV, and context setup
// For external APIs, create robust, realistic mocks in shared helpers if not present
...
describe('API Service E2E Tests', () => {
  ...
  let mockExchangeAPI: any;

  beforeEach(async () => {
    const testDbResult = await getTestDb();
    db = testDbResult.db;
    kv = testDbResult.kv as ExtendedKVNamespace;
    ...
    mockDatabase = {
      users: {
        ...
        findByTelegramId: vi.fn().mockResolvedValue(testUser),
    ...
    // Setup mock exchange API
    mockExchangeAPI = {
      getPrice: vi.fn().mockResolvedValue(45000),
      getBalance: vi.fn().mockResolvedValue({ USDT: 1000 }),
      createOrder: vi.fn().mockResolvedValue({ id: 'order-123', status: 'filled' })
    };
  });
  ...
  describe('Authentication Endpoints', () => {
    ...
    describe('POST /auth/refresh', () => {
      it('should refresh valid token', async () => {
        ...
        // mockJWT.sign.mockReturnValue(newToken); // This line is removed as per the new_code
      ...
      it('should reject expired token', async () => {
        ...
        // mockJWT.verify.mockImplementation(() => { // This line is removed as per the new_code
        //   throw new Error('Token expired'); // This line is removed as per the new_code
  ...
  describe('User Management Endpoints', () => {
    beforeEach(() => {
      ...
      // mockJWT.verify.mockReturnValue({ userId: testUser.id }); // This line is removed as per the new_code
    });
  ...
  describe('Trading Endpoints', () => {
    ...
    describe('GET /trading/positions', () => {
      it('should return user positions', async () => {
        ...
        mockExchangeAPI.getPrice.mockImplementation((symbol: string) => {
          const prices: Record<string, number> = { 'BTC/USDT': 46000, 'ETH/USDT': 3100 };
